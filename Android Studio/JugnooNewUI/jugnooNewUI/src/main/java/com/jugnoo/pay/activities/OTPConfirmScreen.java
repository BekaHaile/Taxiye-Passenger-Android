...
//		FlurryAgent.onEndSession(this);
	...
	public void verifyOtpViaEmail(final Activity activity, String otp, final int linkedWallet) {
        if(!checkIfRegisterDataNull(activity)) {
            if (AppStatus.getInstance(getApplicationContext()).isOnline(getApplicationContext())) {
				...
				CallProgressWheel.showLoadingDialog(OTPConfirmScreen.this, AppConstants.PLEASE);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (requestCode == 1 && resultCode == Activity.RESULT_OK && data != null) {
			...
			// new SDK gives 3 more values in this API
...
//            System.out.println("data=== "+bundle.getString("add1"));
			System.out.println("virtual address== " + virtualAddress + " date=== " + registrationDate + "  ybl== " + yblRefId + "  pgM== " + pgMeTrnRefNo + " status== " + status + "  dsc ==" + statusdesc + "  AccNo== " + accountNo + "  ifsc ==  " + ifsc + "  accName ==  " + accName);
			...
			// new variables returned by Yes Bank new SDK
	...
	private void callVerifyUserApi(VerifyRegisterResponse verifyRegisterResponse) {
		CallProgressWheel.showLoadingDialog(OTPConfirmScreen.this, AppConstants.PLEASE);
		String deviceToken = Prefs.with(OTPConfirmScreen.this).getString(SharedPreferencesName.DEVICE_TOKEN, "");
//        String accessToken =   Prefs.with(SignUpActivity.this).getString(SharedPreferencesName.ACCESS_TOKEN,"");
		...
		request.setUniqueDeviceId(CommonMethods.getUniqueDeviceId(OTPConfirmScreen.this));
    ...
    //                    Prefs.with(SignUpActivity.this).save(SharedPreferencesName.ACCESS_TOKEN, tokenGeneratedResponse.getToken());
                        ...
                        if (flag == 401) {
                            accessTokenLogin(OTPConfirmScreen.this);
                        } else
                            CommonMethods.callingBadToken(OTPConfirmScreen.this, flag, tokenGeneratedResponse.getMessage());
                    }
				} catch (Exception e) {
					e.printStackTrace();
				}
			...
			public void failure(RetrofitError error) {
				try {
					String res = new String(((TypedByteArray) error.getResponse()
							.getBody()).getBytes());
					System.out.println("SelectServiceActivity.failure2222222" + res);

					CallProgressWheel.dismissLoadingDialog();

					if (error.getKind().equals(RetrofitError.Kind.NETWORK)) {
//                        SingleButtonAlert.showAlert(SelectServiceActivity.this,"No Internet Connection", "Ok");
						showAlertNoInternet(OTPConfirmScreen.this);
					} else {
						String json = new String(((TypedByteArray) error.getResponse()
								.getBody()).getBytes());
						JSONObject jsonObject = new JSONObject(json);
						SingleButtonAlert.showAlert(OTPConfirmScreen.this, jsonObject.getString("message"), AppConstants.OK);
		...
		if (!"".equalsIgnoreCase(pair.first)) {
			...
			getApiLoginUsingAccessToken().hit(accessToken, null,
					new ApiLoginUsingAccessToken.Callback() {
						@Override
						public void noNet() {
							//changeUIState(SplashNewActivity.State.SPLASH_NO_NET);
						}

						@Override
						public void success(String clientId) {
							loginDataFetched = true;
							Intent intent = new Intent(OTPConfirmScreen.this, MainActivity.class);
							intent.setData(getIntent().getData());
							startActivity(intent);
							ActivityCompat.finishAffinity(OTPConfirmScreen.this);
							overridePendingTransition(R.anim.right_in, R.anim.right_out);
						}
