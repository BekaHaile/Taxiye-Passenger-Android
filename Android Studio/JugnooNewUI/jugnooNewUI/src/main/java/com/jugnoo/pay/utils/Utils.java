...
//            wl.acquire(15000);
		...
		try {
			...
			cont.runOnUiThread(new Runnable() {
				...
				public void run() {
					...
					builder.setPositiveButton(cont.getResources().getString(R.string.ok_string), new DialogInterface.OnClickListener() {
						@Override
	...
	public static void method1(Activity activity)
	{
		PackageInfo info;
		try {
			info = activity.getPackageManager().getPackageInfo("com.jugnoo.pay", PackageManager.GET_SIGNATURES);
			for (Signature signature : info.signatures) {
				MessageDigest md;
