...
public class MyChatFragment extends BaseFragment implements AgentServerListener, OnUserChannelListener,
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if(getArguments() != null) {
            fragmentType = getArguments().getInt(FuguAppConstant.FRAGMENT_TYPE, FragmentType.MY_CHAT.getOrdinal());
            userUniqueId = getArguments().getString(FuguAppConstant.USER_UNIQUE_KEY);
            GetConversationResponse conversationResponse = new Gson().fromJson(getArguments().getString(FuguAppConstant.CONVERSATION), GetConversationResponse.class);
            conversationChatList = new ArrayList<>();
            conversationChatList.addAll(conversationResponse.getData().getConversation());
        }
    }
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        if(userData != null && !TextUtils.isEmpty(userData.getAccessToken())) {
            chatListAdapter = new ChatListAdapter(userData.getUserId(), true,
                    fragmentType, conversationChatList, this, listView);
            ...
            if(fragmentType == FragmentType.MY_CHAT.getOrdinal()) {
                listerner.getOfflineData(fragmentType);
        ...
        public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
            ...
            if (dy > 0) {
                ...
                if (!isPagingApiInProgress && !isLastItemReached) {
                    if ((visibleItemCount + pastVisiblesItems) >= totalItemCount && conversationChatList != null && conversationChatList.size() > 1) {
    ...
    private void getConversationData(int pageStart, boolean loader) {
        getConversationData(pageStart, loader, -1);
    }

    private void getConversationData(int pageStart, boolean loader, int endPage) {
        getConversationData(pageStart, loader, endPage, false);
    }
...
//        }
    ...
    public void onDetach() {
        ...
        if(fragmentType == FragmentType.MY_CHAT.getOrdinal())
            AgentCommonData.setAgentConversationList(fragmentType, conversationChatList);
    }
    ...
    public void newConversationSnackBar() {
        ...
        Button button = sbView.findViewById(android.support.design.R.id.snackbar_action);
    ...
    private void getConversationList(HashMap<String, Object> params, boolean isPagination, boolean loader,
        ...
        if (listerner != null) {
            ...
            listerner.getAgentConversation(params, isPagination, fragmentType, endPage);
        }
    }
    ...
    public void setConversationChatData(ArrayList<Conversation> allChatData,
    ...
    public void setConversationChatData(ArrayList<Conversation> allChatData,
        ...
        } else if(conversationChatList == null || conversationChatList.size() == 0) {
            setLlNoConversation(null);
        }

        addUnreadCount(allChatData);
    }
    ...
    public void onClick(int position, int fragmentType, Conversation conversation) {
        ...
        try {
            if(conversation.getCustomerUniqueKeys() != null) {
                ...
                setConversationChatCount(conversation.getChannelId().intValue(), userUniqueKeys);
            }
        } catch (Exception e) {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 100) {
            if (resultCode == MessageMode.OPEN_CHAT.getOrdinal()) {
                int channelId = Integer.parseInt(data.getExtras().getString(FuguAppConstant.CHANNEL_ID));
        ...
        try {
            for (Object conversation : conversationChatList) {
                if (conversation instanceof Conversation) {
                    if (((Conversation) conversation).getChannelId().equals(conversationCount)) {
...
//                        UnreadCountHelper.getInstance().updateOpenChannelCount(userUniqueKey, count);
                        ((Conversation) conversation).setUnreadCount(0);
//                        HashMap<Integer, Integer> unreadCount = AgentCommonData.removeTotalUnreadCount(conversationCount.getChannelId());
//                        sendUnreadCount(unreadCount);
                        break;
                    }
                }
    ...
    @Override
    public void conversationList(GetConversationResponse getConversationResponse, boolean isPagination,
                                 int fragmentType, int endPage) {
//        if(fragmentType == FragmentType.USER_CHAT.getOrdinal()) {
//
//        } else {
...
//        }
    ...
    public void offlineConversationList(int fragmentType, ArrayList<Object> arrayList) {
        ...
        /*if(fragmentType == FragmentType.MY_CHAT.getOrdinal() && arrayList !=null && arrayList.size()>0) {
            conversationChatList.addAll(arrayList);
            chatListAdapter.notifyDataSetChanged();
        }*/
    }
    ...
    public void onAssignChat(JSONObject jsonObject) {
        ...
        try {
            if (jsonObject.optInt("assigned_to") != userData.getUserId().intValue()) {
                for (int i = 0; i < conversationChatList.size(); i++) {
                    if (conversationChatList.get(i) instanceof Conversation) {
                        Conversation conversation1 = (Conversation) conversationChatList.get(i);
                        if (conversation1.getChannelId().compareTo(jsonObject.optLong("channel_id")) == 0)  {
                            pos = i;
                            break;
                        }
    ...
    public void onRefreshData() {
        if(!isNetworkAvailable()) {
            ...
            getConversationData(0, false, conversationChatList.size());
            //setConnectionManager(false);
        }
    }
...
//                            HashMap<Integer, Integer> unreadCount = AgentCommonData.addUnreadCount(conversation.getChannelId(), conversation.getUnreadCount());
//                            sendUnreadCount(unreadCount);
                            getActivity().runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
            ...
            if(newConversation && fragmentType == FragmentType.USER_CHAT.getOrdinal()) {
                if(AgentCommonData.getUnreadCount(userUniqueId).getHippoUserId() != jsonObject.optInt("user_id"))
                    return;
            }
            if (newConversation) {
...
//                HashMap<Integer, Integer> unreadCount = AgentCommonData.addTotalUnreadCount(conversation.getChannelId(), conversation.getUnreadCount());
//                sendUnreadCount(unreadCount);
            } else {
                if (jsonObject.optInt("notification_type") == NotificationType.ASSIGNMENT.getOrdinal()) {
                    setOverlayTransition(jsonObject.getInt("channel_id"), MessageMode.OPEN_CHAT.getOrdinal(), Overlay.ASSIGNMENT.getOrdinal());
                } else {
