...
import product.clicklabs.jugnoo.GPSLocationFetcher.MyLocationListener;
...
public class DriverLocationUpdateService extends Service {
	...
	public void onStart(Intent intent, int startId) {
		...
		try {
			...
			if (fast.equalsIgnoreCase("no")) {
				...
				locationFetcherDriver = new LocationFetcherDriver(
						DriverLocationUpdateService.this,
						serverUpdateTimePeriod, 45000);
				...
				gpsLocationFetcher = new GPSLocationFetcher(
						DriverLocationUpdateService.this, accessToken,
						deviceToken, SERVER_URL, serverUpdateTimePeriod);
			}
	...
	public void updateServerData() {
		...
		String LIVE_SERVER_URL = "http://appdemo.clicklabs.in:4010";
		...
		deviceToken = DriverLocationUpdateService.this.getSharedPreferences(
				SplashLogin.class.getSimpleName(), Context.MODE_PRIVATE)
	...
	private boolean isMyServiceRunning() {
		...
		for (RunningServiceInfo service : manager
			...
			if (DriverLocationUpdateService.class.getName().equals(
	...
	public void startCheckIfDriverTimer() {
		...
		try {
			...
			timerTaskCheckIfDriver = new TimerTask() {
				...
				public void run() {
					...
					try {
						...
						Log.e("DriverLocationUpdateService userMode in timertask ",
								"==" + userMode);
			...
			timerCheckIfDriver.scheduleAtFixedRate(timerTaskCheckIfDriver, 10,
					120000);
		} catch (Exception e) {
			e.printStackTrace();
		}
...
class GPSLocationFetcher {
	...
	static long lastServerUpdate;

	Handler callServer = new Handler();

	Runnable updateServer = new Runnable() {
		@Override
		public void run() {
			if (System.currentTimeMillis() - lastServerUpdate > 60000) {
				sendLocationToServer(location);
			}
			callServer.postDelayed(updateServer, 60000);
		}
	};
	 ...
	 * @param context
	 */
	public GPSLocationFetcher(Context context, String accessToken,
			String deviceToken, String SERVER_URL, long updatePeriod) {
		...
		if (locationManager.isProviderEnabled(LocationManager.GPS_PROVIDER)) {
			...
			locationManager.requestLocationUpdates(
					LocationManager.GPS_PROVIDER, updatePeriod, 0, gpsListener);
		} else {
			if (locationManager
					.isProviderEnabled(LocationManager.NETWORK_PROVIDER)) {
				networkListener = new MyLocationListener();
				locationManager.requestLocationUpdates(
						LocationManager.NETWORK_PROVIDER, updatePeriod, 0,
						networkListener);
			}
		}
	...
	class MyLocationListener implements LocationListener {
		...
		public void onLocationChanged(Location loc) {
			if (isBetterLocation(loc, GPSLocationFetcher.this.location)) {
				GPSLocationFetcher.lastServerUpdate = System
						.currentTimeMillis();
				Log.e("************************************** custom",
						"Location changed " + loc);
		...
		new Thread(new Runnable() {
			...
			public void run() {
				try {
					...
					Log.e("DriverLocationUpdateService userMode in PI ", "=="
							+ userMode);

					if (Database2.UM_DRIVER.equalsIgnoreCase(userMode)) {
						if ((wakeLock != null) && // we have a WakeLock
								(wakeLock.isHeld() == false)) { // but we don't
																// hold it
							wakeLock.acquire();
						}
						...
						String result = simpleJSONParser.getJSONFromUrlParams(
								SERVER_URL + "/update_driver_location",
								nameValuePairs);
